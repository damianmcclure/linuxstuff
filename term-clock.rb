...
CHARACTERS = ROOT ? File.join(%w(/ usr share term-clock characters.txt)) : File.join(CURDIR, %w(term-clock characters.txt))
QUOTE = ROOT ? File.join(%w(/ usr share term-clock quotes.txt)) : File.join(CURDIR, %w(term-clock quotes.txt))
CONFIGURATION = ROOT ? File.join(%w(/ usr share term-clock term-clock.conf)) : File.join(CURDIR, %w(term-clock term-clock.conf))
...
def main
	...
	characters = IO.read(CHARACTERS, :encoding => "UTF-8").split(/#+/).reject!(&:empty?).reduce({}) do |x, y|
	...
	if display_quote
		if File.readable?(QUOTE)
			quotes = IO.readlines(QUOTE, :encoding => "UTF-8").uniq.map! { |x| x.split(?\t).values_at(1, 0).each(&:strip!).join("    -".freeze) }
